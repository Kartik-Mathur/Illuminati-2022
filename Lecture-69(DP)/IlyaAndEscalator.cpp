...
void ilyaAndEscalator() {
	...
	for (int time = 1; time <= t; ++time)
	{
		...
		{
			if (people == 0) {
				dp[time][people] = (1 - p) * dp[time - 1][people];
			}
			else if (people == n) {
				dp[time][people] = dp[time - 1][people] + p * dp[time - 1][people - 1];
			}
			else {
				dp[time][people] = (1 - p) * dp[time - 1][people] + p * dp[time - 1][people - 1];
			}
		}
	}
