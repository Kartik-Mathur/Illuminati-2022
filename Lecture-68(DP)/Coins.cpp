...
double coins(ll n, ll heads) {
	// base case
	if (heads == 0) return dp[n][heads] = 1;
	if (n == 0) return dp[n][heads] = 0;
	if (dp[n][heads] > -1) return dp[n][heads];
	// recursive case
	return dp[n][heads] = p[n - 1] * coins(n - 1, heads - 1) +
	                      (1 - p[n - 1]) * coins(n - 1, heads);
}
...
void solve() {
	...
	cout << fixed << setprecision(9) << coins(n, n / 2 + 1);
}
